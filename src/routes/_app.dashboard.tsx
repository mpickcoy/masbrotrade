...
function Dashboard() {
  ...
  const saveMoodBusy = useRef(false);
  ...
  // Debounced: ignore rapid double-clicks; DB is single source of truth (no localStorage)
  const saveMood = (v: string) => {
    if (saveMoodBusy.current) return;
    saveMoodBusy.current = true;
    setMood(v);
    const today = new Date().toLocaleDateString("sv-SE");
    updateProfile({ mood: `${today}|${v}` }, {
      onSettled: () => { saveMoodBusy.current = false; },
    });
  };
  ...
  const computeSessions = useCallback(() => {
    ...
    // UTC offset sesi forex (standard time, no DST adjustment needed for WIB reference):
    // Asia/Tokyo:   UTC 00:00–09:00  → WIB 07:00–16:00
    // London:       UTC 07:00–16:00  → WIB 14:00–23:00  (BST: UTC+1, GMT: UTC+0 → pakai UTC 07 agar stabil)
    // New York:     UTC 12:00–21:00  → WIB 19:00–04:00  (EDT: UTC-4, EST: UTC-5 → pakai UTC 12 agar stabil)
    // Catatan: waktu London/NY disesuaikan ke versi tanpa DST agar label WIB konsisten sepanjang tahun.
    const sessions = [
      { name: "Asia",     start: 0 * 60,  end: 9 * 60,  color: "#5ee7df", emoji: "🌏", timeLabel: "07:00 – 16:00 WIB" },
      { name: "London",   start: 7 * 60,  end: 16 * 60, color: "#b490f5", emoji: "🏛️", timeLabel: "14:00 – 23:00 WIB" },
      { name: "New York", start: 12 * 60, end: 21 * 60, color: "#a8f08a", emoji: "🗽", timeLabel: "19:00 – 04:00 WIB" },
    ];
  ...
  // Memoized: recompute only when filteredCurve changes, not on every hover/render
