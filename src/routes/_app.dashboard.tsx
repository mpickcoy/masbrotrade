  ...
  return (
    <>
      <style>{`
        ...
        .db-date{display:flex;align-items:center;gap:6px;background:rgba(13,20,36,.75);border:1px solid rgba(255,255,255,.06);
          border-radius:10px;padding:7px 14px;font-size:.7rem;color:#5a6585;backdrop-filter:blur(12px);white-space:nowrap;}

        /* STAT CARDS */
        .db-cards{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px;}
        @keyframes cardIn{from{opacity:0;transform:translateY(22px) rotateX(8deg);}to{opacity:1;transform:translateY(0) rotateX(0);}}
        .db-card{position:relative;border-radius:20px;padding:22px 20px;border:1px solid rgba(255,255,255,.08);
          overflow:hidden;cursor:pointer;transition:transform .3s cubic-bezier(.34,1.56,.64,1),box-shadow .3s;animation:cardIn .55s ease both;}
        .db-card:nth-child(1){animation-delay:.05s} .db-card:nth-child(2){animation-delay:.12s}
        .db-card:nth-child(3){animation-delay:.19s} .db-card:nth-child(4){animation-delay:.26s}
        .db-card:hover{transform:translateY(-5px) scale(1.02);}
        .db-card::after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.04),transparent 50%);border-radius:inherit;pointer-events:none;}
        .db-card-orb{position:absolute;top:-40px;right:-40px;width:110px;height:110px;border-radius:50%;filter:blur(38px);opacity:.5;pointer-events:none;transition:opacity .3s;}
        ...
        .db-card-label{font-size:.66rem;letter-spacing:.1em;text-transform:uppercase;margin-bottom:10px;display:flex;align-items:center;gap:7px;}
        ...
        .db-card-value{font-family:'Syne',sans-serif;font-weight:800;font-size:1.8rem;letter-spacing:-1px;line-height:1;margin-bottom:8px;color:#e8edf8;}
        .db-card-sub{font-size:.66rem;color:#5a6585;display:flex;align-items:center;gap:6px;flex-wrap:wrap;}
        .db-badge-up{color:#00ffa3;} .db-badge-down{color:#ff4d6d;}

        /* AI BAR */
        @keyframes aiPulse{0%,100%{box-shadow:0 4px 14px rgba(0,255,163,.3);}50%{box-shadow:0 4px 24px rgba(0,255,163,.6);}}
        .db-ai-bar{margin-bottom:24px;background:rgba(13,20,36,.75);border:1px solid rgba(255,255,255,.06);
          border-radius:16px;padding:14px 18px;display:flex;align-items:center;gap:12px;
          backdrop-filter:blur(20px);transition:border-color .3s,box-shadow .3s;}
        .db-ai-bar:focus-within{border-color:rgba(0,255,163,.35);box-shadow:0 0 0 3px rgba(0,255,163,.06);}
        .db-ai-indicator{width:34px;height:34px;border-radius:10px;flex-shrink:0;
          background:linear-gradient(135deg,#00ffa3,#00cfff);display:flex;align-items:center;
          justify-content:center;font-size:.72rem;font-weight:700;color:#000;animation:aiPulse 3s ease-in-out infinite;}
        .db-ai-input{flex:1;min-width:0;background:transparent;border:none;outline:none;
          color:#e8edf8;font-family:'DM Mono',monospace;font-size:.78rem;}
        .db-ai-input::placeholder{color:#5a6585;}
        .db-ai-send{background:linear-gradient(135deg,#00ffa3,#00cfff);border:none;border-radius:9px;
          padding:8px 15px;color:#000;font-family:'Syne',sans-serif;font-weight:700;font-size:.7rem;
          cursor:pointer;letter-spacing:.04em;white-space:nowrap;flex-shrink:0;transition:opacity .2s,transform .15s;}
        .db-ai-send:hover{opacity:.85;transform:scale(.97);}

        /* BOTTOM GRID */
        .db-bottom-grid{display:grid;grid-template-columns:1fr 360px;gap:16px;}
        @keyframes cardInD{from{opacity:0;transform:translateY(22px);}to{opacity:1;transform:translateY(0);}}
        .db-chart-card{border-radius:20px;padding:24px 26px;background:rgba(13,20,36,.75);border:1px solid rgba(255,255,255,.06);
          backdrop-filter:blur(20px);animation:cardInD .55s .3s ease both;}
        ...
        .db-tab{padding:5px 11px;border-radius:7px;font-size:.66rem;letter-spacing:.06em;text-transform:uppercase;
          cursor:pointer;color:#5a6585;background:transparent;border:1px solid transparent;transition:all .2s;font-family:'DM Mono',monospace;}
        .db-tab.active{background:rgba(0,255,163,.12);color:#00ffa3;border-color:rgba(0,255,163,.22);}
        .db-tab:hover:not(.active){color:#e8edf8;}
        .db-chart-area{fill:url(#dbAreaGrad);}
        .db-chart-line{fill:none;stroke:#00ffa3;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round;}
        ...
        /* FEED */
        .db-feed-card{border-radius:20px;padding:24px 22px;background:rgba(13,20,36,.75);border:1px solid rgba(255,255,255,.06);
          backdrop-filter:blur(20px);animation:cardInD .55s .38s ease both;}
        .db-feed-title{font-family:'Syne',sans-serif;font-weight:700;font-size:.95rem;margin-bottom:16px;color:#e8edf8;}
        .db-trade-item{display:flex;align-items:center;gap:11px;padding:11px 0;border-bottom:1px solid rgba(255,255,255,.04);
        ...
        .db-trade-pnl.pos{color:#00ffa3;} .db-trade-pnl.neg{color:#ff4d6d;}
      ...
      <div className="db-wrap">
        ...
        <div className="db-topbar">
          <h1 className="db-title">Halo, <span>{profile?.full_name?.split(" ")[0] ?? "Trader"}</span> 👋</h1>
        ...
        <div className="db-cards">
          ...
          <div className="db-card db-card-winrate">
            ...
            <div className="db-card-sub">
              <span className="db-badge-up">▲</span><span>{stats.tradeCount} trade</span>
            </div>
          </div>
          <div className="db-card db-card-trades">
            ...
            <div className="db-card-sub">
              <span className="db-badge-up">▲</span><span>semua waktu</span>
            </div>
          </div>
        ...
        <div className="db-ai-bar">
          ...
          <input className="db-ai-input" type="text" placeholder="Ketik trade kamu… cth: Long BTC entry 65000 exit 66000 lot 0.1" />
          <button className="db-ai-send">CATAT ↗</button>
        </div>
        ...
        <div className="db-bottom-grid">
          ...
          <div className="db-chart-card">
            <div className="db-chart-header">
              ...
              <div className="db-tabs">
                {["1M","3M","6M","1Y"].map((t) => (
                  <button key={t} className={`db-tab${t==="3M"?" active":""}`}>{t}</button>
                ))}
              </div>
            </div>
            <svg width="100%" height="190" viewBox="0 0 600 200" preserveAspectRatio="none">
              ...
              {curve.length > 1 ? (() => {
                ...
                return (
                  <>
                    <path className="db-chart-area" d={`M${pts.replace(/ /g," L")} L600,200 L0,200 Z`}/>
                    <polyline className="db-chart-line" points={pts}/>
                    <circle className="db-chart-dot" cx={firstPt.split(",")[0]} cy={firstPt.split(",")[1]} r="4"/>
                    <circle className="db-chart-dot" cx={lastPt.split(",")[0]}  cy={lastPt.split(",")[1]}  r="5"/>
                  </>
                );
              })() : (
                <>
                  <path className="db-chart-area" d="M0,160 C100,130 200,110 300,80 C400,50 500,35 600,15 L600,200 L0,200 Z"/>
                  <polyline className="db-chart-line" points="0,160 150,120 300,80 450,45 600,15"/>
          ...
          <div className="db-feed-card">
            ...
            {trades.length === 0 ? (
              <p style={{fontSize:".78rem",color:"#5a6585"}}>Belum ada trade. Gunakan asisten AI di atas.</p>
            ) : trades.slice(0, 5).map((t) => (
              <div key={t.id} className="db-trade-item">
                <div className={`db-trade-dir ${Number(t.pnl) >= 0 ? "long" : "short"}`}>
                  {Number(t.pnl) >= 0 ? "L" : "S"}
                </div>
                <div className="db-trade-info">
                  <div className="db-trade-pair">{t.pair}</div>
                  <div className="db-trade-meta">{new Date(t.traded_at).toLocaleDateString("id-ID")}</div>
                </div>
