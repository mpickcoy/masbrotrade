...
function Dashboard() {
  ...
  const css = `
    ...
    @media (max-width:640px) {
      /* Layout */
      .db-wrap    { padding:12px 12px 100px; }

      /* Topbar */
      .db-topbar  { margin-bottom:14px; flex-wrap:wrap; gap:6px; }
      .db-title   { font-size:1.05rem; }
      .db-date    { display:none; }

      /* Stat cards — 2 kolom, lebih compact */
      .db-cards       { grid-template-columns:repeat(2,1fr); gap:8px; margin-bottom:12px; }
      .db-card        { padding:13px 12px; border-radius:14px; }
      .db-card-value  { font-size:1.15rem; letter-spacing:-.5px; }
      .db-card-label  { font-size:.58rem; margin-bottom:6px; }
      .db-card-sub    { font-size:.58rem; }

      /* AI Bar — sembunyikan di mobile (pakai FAB) */
      .db-ai-bar { display:none; }

      /* Reminder */
      .db-reminder      { flex-direction:column; align-items:flex-start; gap:8px; padding:11px 14px; }
      .db-reminder-btn  { align-self:stretch; text-align:center; }

      /* Mood */
      .db-mood-card   { flex-direction:column; align-items:flex-start; padding:13px 14px; gap:10px; }
      .db-mood-opts   { width:100%; gap:6px; }
      .db-mood-btn    { padding:9px 4px; font-size:.58rem; }
      .db-mood-emoji  { font-size:1.1rem; }

      /* Target harian */
      .db-target-card   { padding:14px 14px; margin-bottom:12px; }
      .db-target-title  { font-size:.82rem; }
      .db-target-row    { flex-wrap:wrap; gap:8px; }
      .db-target-input  { min-width:0; flex:1 1 120px; font-size:.76rem; padding:8px 10px; }
      .db-target-vals   { font-size:.65rem; }

      /* Streak + daily bars */
      .db-extra-row   { grid-template-columns:1fr; gap:8px; margin-bottom:12px; }
      .db-streak-card { padding:16px; flex-direction:row; align-items:center; gap:14px; }
      .db-streak-val  { font-size:2rem; }
      .db-bars-card   { padding:14px; }
      .db-bars-inner  { height:52px; gap:4px; }
      .db-bar-day     { font-size:.55rem; }

      /* Top pairs + quote */
      .db-pairs-quote-row { grid-template-columns:1fr; gap:8px; margin-bottom:12px; }
      .db-pairs-card      { padding:14px 14px; }
      .db-pairs-title     { font-size:.82rem; margin-bottom:10px; }
      .db-pair-row        { padding:7px 0; gap:8px; }
      .db-pair-name       { font-size:.74rem; }
      .db-pair-wr         { font-size:.6rem; display:none; }
      .db-pair-pnl        { font-size:.74rem; }
      .db-quote-card      { padding:14px; min-height:auto; }
      .db-quote-icon      { font-size:1.1rem; margin-bottom:6px; }
      .db-quote-text      { font-size:.74rem; line-height:1.5; }
      .db-quote-author    { font-size:.6rem; margin-top:8px; }

      /* Sesi trading — scroll horizontal di mobile */
      .db-sesi-row  { grid-template-columns:repeat(3,minmax(140px,1fr)); overflow-x:auto; padding-bottom:4px; margin-bottom:12px; }
      .db-sesi-card { padding:12px 13px; gap:5px; }
      .db-sesi-name { font-size:.76rem; }
      .db-sesi-badge{ font-size:.54rem; padding:2px 6px; }
      .db-sesi-time { font-size:.58rem; }
      .db-sesi-timeleft { font-size:.58rem; }

      /* Max loss */
      .db-maxloss-card    { padding:14px 14px; margin-bottom:12px; }
      .db-maxloss-title   { font-size:.82rem; }
      .db-maxloss-vals    { font-size:.63rem; flex-direction:column; gap:3px; }
      .db-maxloss-alert   { font-size:.7rem; padding:9px 12px; }
      .db-maxloss-input   { font-size:.76rem; padding:8px 10px; }

      /* RR + Pie */
      .db-rr-pie-row  { grid-template-columns:1fr; gap:8px; margin-bottom:12px; }
      .db-rr-card     { padding:14px 14px; }
      .db-rr-title    { font-size:.82rem; margin-bottom:12px; }
      .db-rr-grid     { gap:8px; }
      .db-rr-item     { padding:10px 11px; border-radius:10px; }
      .db-rr-lbl      { font-size:.57rem; }
      .db-rr-val      { font-size:1.05rem; }
      .db-pie-card    { padding:14px 14px; }
      .db-pie-title   { font-size:.82rem; margin-bottom:12px; }
      .db-pie-inner   { gap:14px; }
      .db-pie-leg     { font-size:.67rem; }

      /* Bottom grid */
      .db-bottom-grid { grid-template-columns:1fr; gap:8px; }
      .db-chart-card  { padding:14px 12px; border-radius:16px; }
      .db-chart-title { font-size:.85rem; }
      .db-tab         { padding:4px 9px; font-size:.6rem; }
      .db-feed-card   { padding:14px 12px; border-radius:16px; }
      .db-feed-title  { font-size:.85rem; margin-bottom:12px; }
      .db-trade-item  { gap:9px; padding:10px 0; }
      .db-trade-dir   { width:30px; height:30px; font-size:.65rem; border-radius:8px; }
      .db-trade-pair  { font-size:.74rem; }
      .db-trade-meta  { font-size:.59rem; }
      .db-trade-pnl   { font-size:.78rem; }
    }

    /* ── Small phones (≤380px) ── */
    @media (max-width:380px) {
      .db-wrap          { padding:10px 10px 100px; }
      .db-cards         { gap:6px; }
      .db-card          { padding:11px 10px; border-radius:12px; }
      .db-card-value    { font-size:1rem; }
      .db-sesi-row      { grid-template-columns:repeat(3,minmax(120px,1fr)); }
      .db-rr-grid       { grid-template-columns:1fr 1fr; }
      .db-mood-btn      { padding:7px 3px; }
    }
  `;
