...
function Dashboard() {
  ...
  // ── Target harian dari Supabase profile ──
  ...
  const [mood, setMood] = useState<string>(() => {
    // Load mood dari profile jika sudah tersimpan hari ini
  ...
  // Filter curve by active tab — using actual date comparison
  const filteredCurve = useMemo(() => {
    ...
    // Always include index 0 ("Start"), filter rest by date
    return curve.filter((point, i) => {
      if (i === 0) return true; // always keep "Start" anchor
      // curve points after index 0 have dates stored as traded_at strings
      // We use the raw trades/moves dates from equityCurve — compare by position
      // equityCurve returns points in chronological order, so we can slice from cutoff
      const pointDate = new Date(point.date);
      // If date is invalid (e.g. "Start" label already handled above), keep it
  ...
  // ── Sesi Trading (Asia/London/NY) — refresh setiap menit ──
  ...
  // ── Sync mood dari profile Supabase saat data load ──
  useEffect(() => {
  ...
  // ── Max Loss Harian ──
  // ── Max Loss Harian dari Supabase profile ──
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    .db-wrap { padding: 32px 36px; overflow-x: hidden; width: 100%; max-width: 100%; }
    ...
    /* STAT CARDS */
    .db-cards { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:24px; width:100%; }
    .db-card  {
      ...
      animation:db-cardIn .55s ease both; min-width:0;
    }
    ...
    .db-pairs-quote-row > * { min-width:0; }
    ...
    @media (max-width:1024px) {
      ...
      .db-sesi-row        { grid-template-columns:repeat(3,1fr); }
    }

    /* ── Mobile (≤640px) ── */
    @media (max-width:640px) {
      /* Base */
      .db-wrap { padding:12px 12px 110px; width:100%; overflow-x:hidden; }

      /* Topbar */
      .db-topbar { margin-bottom:12px; }
      .db-title  { font-size:1rem; }
      .db-date   { display:none; }

      /* ── STAT CARDS: 2 kolom full width, lebih pendek ── */
      .db-cards      { grid-template-columns:1fr 1fr; gap:8px; margin-bottom:10px; width:100%; }
      .db-card       { padding:14px 13px; border-radius:14px; min-width:0; }
      .db-card-label { font-size:.58rem; margin-bottom:5px; gap:5px; }
      .db-card-dot   { width:5px; height:5px; }
      .db-card-value { font-size:1.2rem; letter-spacing:-.3px; margin-bottom:5px; }
      .db-card-sub   { font-size:.58rem; gap:4px; }

      /* AI Bar */
      .db-ai-bar { display:none; }

      /* Reminder */
      .db-reminder      { flex-direction:column; align-items:flex-start; gap:8px; padding:10px 12px; margin-bottom:10px; border-radius:12px; }
      .db-reminder-text { font-size:.72rem; }
      .db-reminder-btn  { width:100%; text-align:center; padding:8px 14px; }

      /* ── MOOD: row horizontal, lebih pendek ── */
      .db-mood-card  { padding:11px 12px; margin-bottom:10px; gap:10px; flex-wrap:wrap; border-radius:14px; }
      .db-mood-label { font-size:.68rem; width:100%; }
      .db-mood-opts  { gap:6px; flex:1; width:100%; }
      .db-mood-btn   { padding:8px 4px; font-size:.58rem; border-radius:10px; min-width:0; flex:1; }
      .db-mood-emoji { font-size:1rem; }

      /* ── TARGET: compact ── */
      .db-target-card   { padding:13px 12px; margin-bottom:10px; border-radius:14px; }
      .db-target-title  { font-size:.82rem; }
      .db-target-vals   { font-size:.64rem; flex-wrap:wrap; gap:2px; }
      .db-reached-badge { font-size:.6rem; padding:2px 8px; }
      .db-target-bar-bg { height:6px; }
      .db-target-row    { gap:7px; }
      .db-target-input  { font-size:.76rem; padding:7px 10px; }

      /* ── STREAK + BARS: 1 kolom, streak jadi horizontal ringkas ── */
      .db-extra-row   { grid-template-columns:1fr; gap:8px; margin-bottom:10px; }
      .db-streak-card { padding:14px 14px; flex-direction:row; align-items:center; gap:12px; border-radius:14px; }
      .db-streak-label{ font-size:.6rem; }
      .db-streak-val  { font-size:1.8rem; letter-spacing:-1px; }
      .db-streak-fire { font-size:1.2rem; }
      .db-streak-sub  { font-size:.6rem; }
      .db-bars-card   { padding:12px 13px; border-radius:14px; }
      .db-bars-title  { font-size:.82rem; }
      .db-bars-header { margin-bottom:10px; }
      .db-bars-inner  { height:48px; gap:4px; }
      .db-bar-day     { font-size:.54rem; }

      /* ── TOP PAIRS + QUOTE: 1 kolom ── */
      .db-pairs-quote-row { grid-template-columns:1fr; gap:8px; margin-bottom:10px; }
      .db-pairs-card      { padding:13px 13px; border-radius:14px; }
      .db-pairs-title     { font-size:.82rem; margin-bottom:10px; }
      .db-pair-row        { padding:7px 0; gap:8px; }
      .db-pair-name       { font-size:.74rem; }
      .db-pair-wr         { display:none; }
      .db-pair-pnl        { font-size:.74rem; }
      .db-quote-card      { padding:13px 13px; border-radius:14px; min-height:auto; }
      .db-quote-icon      { font-size:1rem; margin-bottom:5px; }
      .db-quote-text      { font-size:.72rem; line-height:1.5; }
      .db-quote-author    { font-size:.6rem; margin-top:7px; }

      /* ── SESI: scroll horizontal, jangan stack ── */
      .db-sesi-row {
      ...
      .db-sesi-card {
        ...
        padding:11px 12px; border-radius:13px; gap:5px; min-width:0;
      }
      .db-sesi-name  { font-size:.74rem; gap:5px; }
      .db-sesi-badge { font-size:.52rem; padding:2px 6px; }
      .db-sesi-time  { font-size:.57rem; }
      .db-sesi-timeleft { font-size:.57rem; }

      /* ── MAX LOSS: compact ── */
      .db-maxloss-card  { padding:13px 12px; margin-bottom:10px; border-radius:14px; }
      .db-maxloss-title { font-size:.82rem; }
      .db-maxloss-vals  { font-size:.63rem; flex-wrap:wrap; gap:3px; }
      .db-maxloss-bar-bg{ height:6px; }
      .db-maxloss-alert { font-size:.69rem; padding:8px 11px; }
      .db-maxloss-input { font-size:.76rem; padding:7px 10px; }

      /* ── RR + PIE: 1 kolom, lebih compact ── */
      .db-rr-pie-row { grid-template-columns:1fr; gap:8px; margin-bottom:10px; }
      .db-rr-card    { padding:13px 13px; border-radius:14px; }
      .db-rr-title   { font-size:.82rem; margin-bottom:10px; }
      .db-rr-grid    { gap:7px; }
      .db-rr-item    { padding:9px 10px; border-radius:9px; }
      .db-rr-lbl     { font-size:.56rem; margin-bottom:4px; }
      .db-rr-val     { font-size:1.05rem; }
      .db-rr-sub     { font-size:.56rem; }
      .db-pie-card   { padding:13px 13px; border-radius:14px; }
      .db-pie-title  { font-size:.82rem; margin-bottom:10px; }
      .db-pie-inner  { gap:12px; }
      .db-pie-leg    { font-size:.65rem; gap:6px; }

      /* ── BOTTOM GRID ── */
      .db-bottom-grid { grid-template-columns:1fr; gap:8px; }
      .db-chart-card  { padding:13px 12px; border-radius:14px; }
      .db-chart-title { font-size:.82rem; }
      .db-chart-header{ margin-bottom:14px; }
      .db-tab         { padding:4px 8px; font-size:.58rem; }
      .db-feed-card   { padding:13px 12px; border-radius:14px; }
      .db-feed-title  { font-size:.82rem; margin-bottom:10px; }
      .db-trade-item  { gap:8px; padding:9px 0; }
      .db-trade-dir   { width:28px; height:28px; font-size:.62rem; border-radius:7px; }
      .db-trade-pair  { font-size:.73rem; }
      .db-trade-meta  { font-size:.58rem; }
      .db-trade-pnl   { font-size:.76rem; }
    }

    /* ── Kecil (≤390px) ── */
    @media (max-width:390px) {
      .db-wrap        { padding:10px 10px 110px; }
      .db-card        { padding:11px 10px; }
      .db-card-value  { font-size:1.05rem; }
      .db-sesi-card   { flex:0 0 56%; }
      .db-rr-val      { font-size:.95rem; }
      .db-streak-val  { font-size:1.6rem; }
    }
  `;
  ...
  return (
    <>
      ...
      <div className="db-wrap">
        ...
        {isLoading && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
              {[...Array(4)].map((_, i) => (
                <div key={i} style={{ borderRadius:20, padding:"22px 20px", background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.06)", animation:"db-cardIn .5s ease both", animationDelay:`${i*.1}s` }}>
                  <div style={{ height:10, width:"50%", borderRadius:6, background:"rgba(255,255,255,.06)", marginBottom:14 }} />
                  <div style={{ height:28, width:"75%", borderRadius:8, background:"rgba(255,255,255,.08)", marginBottom:10 }} />
                  <div style={{ height:8, width:"40%", borderRadius:6, background:"rgba(255,255,255,.04)" }} />
                </div>
              ))}
            </div>
            <div style={{ borderRadius:20, height:220, background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.06)" }} />
          </div>
        )}
        ...
        {!isLoading && isError && (
          <div style={{
            borderRadius:16, padding:"24px 20px", background:"rgba(255,77,109,.07)",
            border:"1px solid rgba(255,77,109,.25)", color:"#ff4d6d",
            display:"flex", alignItems:"center", gap:12, marginBottom:24,
          }}>
            <span style={{ fontSize:"1.5rem" }}>⚠️</span>
            <div>
              <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, marginBottom:4 }}>Gagal memuat data</div>
              <div style={{ fontSize:".76rem", color:"#c86070" }}>Periksa koneksi internet Anda, lalu refresh halaman.</div>
            </div>
            <button
              type="button"
              onClick={() => window.location.reload()}
              style={{ marginLeft:"auto", background:"rgba(255,77,109,.15)", border:"1px solid rgba(255,77,109,.3)", borderRadius:9, padding:"8px 16px", color:"#ff4d6d", fontWeight:700, fontSize:".75rem", cursor:"pointer", whiteSpace:"nowrap" }}
            >
              Refresh ↺
            </button>
          </div>
        ...
        {/* ── Main Dashboard — only when loaded ── */}
        {!isLoading && !isError && (<>
          ...
          <div className="db-chart-card">
            ...
            <svg width="100%" height="190" viewBox="0 0 600 200" preserveAspectRatio="none" style={{overflow:"visible"}}>
              ...
              {svgPath ? (
                ...
                <g>
                  <text x="300" y="85" textAnchor="middle" fill="#5a6585" fontSize="13" fontFamily="DM Mono,monospace">📈</text>
                  <text x="300" y="110" textAnchor="middle" fill="#5a6585" fontSize="11" fontFamily="DM Mono,monospace">Belum ada data trade.</text>
                  <text x="300" y="130" textAnchor="middle" fill="#3a4260" fontSize="10" fontFamily="DM Mono,monospace">Catat trade pertama untuk melihat equity curve.</text>
                </g>
              )}
