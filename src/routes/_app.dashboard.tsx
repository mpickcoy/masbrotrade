...
function StatCard({
  ...
  const accentClass =
    ...
    accent === "loss"   ? "text-loss"    : "text-foreground";
  return (
    <Card className="p-4">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard
          label="Saldo"
          value={fmtMoney(stats.balance, currency)}
          sub={`Modal ${fmtMoney(profile?.initial_capital ?? 0, currency)}`}
          accent="primary"
        />
        <StatCard
          label="Total P/L"
          value={fmtMoney(stats.totalPnl, currency)}
          sub={`${stats.tradeCount} trade`}
          accent={stats.totalPnl >= 0 ? "profit" : "loss"}
        />
        <StatCard label="Total Loss" value={fmtMoney(stats.totalLoss, currency)} accent="loss" />
        <StatCard
          label="Win Rate"
          value={`${stats.winRate.toFixed(1)}%`}
          sub={`Profit ${fmtMoney(stats.totalProfit, currency)}`}
          accent="profit"
        />
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium">
                      {t.pair}{" "}
                      <span className="text-xs uppercase text-muted-foreground">{t.side}</span>
                    </div>
